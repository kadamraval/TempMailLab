...
/**
 * Creates or updates a user document in Firestore. This is an idempotent action.
 * It uses set with merge:true to safely create the document if it doesn't exist,
 * or update it if it does. This is the primary function for ensuring a user
...
export async function signUp(uid: string, email: string | null, isAnonymous: boolean): Promise<{ success: boolean; error?: string }> {
  ...
  try {
    ...
    // - If doc exists, it's UPDATED with the new email/isAnonymous status.
