...
export function DashboardClient() {
  ...
  useEffect(() => {
    ...
    useEffect(() => {
     ...
     useEffect(() => {
        // This effect detects when a new inbox is created and sets it as active.
        if (isCreating && liveUserInboxes && liveUserInboxes.length > (userInboxesQuery?.length || 0)) {
            const newInbox = liveUserInboxes[0]; // The new inbox should be at the top due to ordering
            if (newInbox && newInbox.id !== activeInbox?.id) {
                setActiveInbox(newInbox);
                if (userProfile?.isAnonymous) {
                    localStorage.setItem(LOCAL_INBOX_KEY, newInbox.id);
                }
                setIsCreating(false); // End the creation process
            }
        }
    }, [liveUserInboxes, isCreating, activeInbox, userProfile]);
